...
import cn.edu.thssdb.schema.Meta;
...
public class UserManager {
  ...
  public UserManager() {
    ...
    try {
      meta = new Meta(Global.DATA_ROOT_FOLDER, "user.meta");
      loadUserData();
    } catch (Exception e) {
  ...
  public UserService getUserService(long sessionId) {
    try {
      lock.readLock().lock();
      return onlineUsers.getOrDefault(sessionId, null); // 若不存在则返回 null, 否则返回 UserService
    } finally {
  ...
  public User.Permission getPermission(long sessionId) {
    try {
      lock.readLock().lock();
      if (onlineUsers.containsKey(sessionId)) return onlineUsers.get(sessionId).user.permission;
      else return User.Permission.NONE;
    } finally {
      lock.readLock().unlock();
    }
  }
  ...
  public long login(String username, String password) {
    // ROOT 用户检测
    if (username.equals(Global.DEFAULT_USER_NAME) && password.equals(Global.DEFAULT_PASSWORD)) {
      User user =
          new User(Global.DEFAULT_USER_NAME, Global.DEFAULT_PASSWORD, User.Permission.ADMIN);
      long sessionId = getRandomSessionId();
      // 如果default用户未注册，则注册
      if (getUser(Global.DEFAULT_USER_NAME) == null) {
        try {
          lock.writeLock().lock();
          users.add(user);
          saveUserData();
      ...
      try {
        lock.writeLock().lock();
        onlineUsers.put(sessionId, new UserService(user, sessionId)); // 登录成功 记录在线用户数据
      } finally {
        lock.writeLock().unlock();
      }
      return sessionId;
  ...
  public boolean logout(long sessionId) {
    ...
    try {
      lock.readLock().lock();
      if (!onlineUsers.containsKey(sessionId)) return false;
    } finally {
      lock.readLock().unlock();
    }
    // 注销成功 删除在线用户数据
  ...
  public long getRandomSessionId() {
    ...
    while (true) {
      ...
      try {
        lock.readLock().lock();
        if (!onlineUsers.containsKey(r)) return r;
      } finally {
        lock.readLock().unlock();
      }
    }
